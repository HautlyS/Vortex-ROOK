...
<template>
  <div class="relative">
    ...
    >
      <div v-if="showDropdown" class="absolute top-full left-0 mt-2 w-80 glass-panel-strong rounded-2xl p-4 z-50 max-h-96 overflow-hidden">
        <input
          v-model="searchQuery"
          type="text"
        ...
        <div class="space-y-4 overflow-y-auto max-h-80">
          <div v-if="systemFonts.length">
            <h4 class="text-xs text-white/50 mb-2 font-medium">System</h4>
            <div class="space-y-1">
              <button
          ...
          <div v-if="googleFonts.length">
            <h4 class="text-xs text-white/50 mb-2 font-medium">Google Fonts</h4>
            <div class="space-y-1">
              <button
          ...
          <div v-if="fontsLoading" class="text-center py-4">
            <div class="animate-spin w-5 h-5 border-2 border-violet-500 border-t-transparent rounded-full mx-auto"></div>
          </div>
        </div>
      </div>
    </Transition>
