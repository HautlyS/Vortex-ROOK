...
<template>
  <BackgroundTexture
    variant="dots"
    :opacity="0.25"
    class="flex h-full flex-col bg-gradient-to-b from-white/[0.02] to-transparent"
  >
    <!-- Header -->
    <div class="flex items-center justify-between px-3 md:px-4 py-2.5 md:py-3 border-b border-white/[0.06]">
      <div class="flex items-center gap-2 md:gap-2.5">
        <div class="w-7 h-7 md:w-8 md:h-8 rounded-lg md:rounded-xl bg-gradient-to-br from-sky-500/25 to-cyan-500/15 flex items-center justify-center ring-1 ring-white/10">
          <svg
            class="w-3.5 h-3.5 md:w-4 md:h-4 text-sky-400"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      <div class="flex items-center gap-2">
        <span
          v-if="layers.length"
          class="text-[10px] md:text-[11px] font-medium text-white/40 bg-white/[0.06] px-2 py-0.5 rounded-md"
        >
          {{ layers.length }}
        </span>
        <button
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      >
        ...
        <p class="text-xs text-white/30 mt-1">
          Import a document to see layers
        </p>
      </div>
        ...
        >
          ...
          <div class="flex-1 min-w-0 pr-1">
            ...
            <div class="flex items-center gap-2 mt-0.5">
              ...
              <span
                v-if="layer.role !== 'content'"
                class="text-[9px] px-1.5 py-px rounded bg-fuchsia-500/20 text-fuchsia-300 font-medium uppercase"
              >
                {{ layer.role }}
              </span>
            </div>
          ...
          <div class="flex items-center gap-0.5 shrink-0">
            ...
            <div class="hidden group-hover:flex items-center gap-0.5 ml-1 pl-1 border-l border-white/10">
              <button
                class="w-6 h-6 rounded flex items-center justify-center text-white/30 hover:text-white/70 hover:bg-white/10"
                @click.stop="moveUp(layer)"
              >
                <svg
                  class="w-3 h-3"
                  fill="none"
              ...
              <button
                v-if="!layer.locked"
                class="w-6 h-6 rounded flex items-center justify-center text-white/30 hover:text-rose-400 hover:bg-rose-500/10"
    ...
    <div
      ...
      class="p-3 border-t border-white/[0.06]"
    >
      <div class="flex gap-2">
        <button
          class="flex-1 h-8 rounded-lg bg-white/[0.05] hover:bg-white/[0.1] text-[11px] font-medium text-white/70 hover:text-white flex items-center justify-center gap-1.5 transition-all"
          @click="bringToFront(layers.find(l => l.id === selectedIds[0])!)"
        >
          <svg
            class="w-3 h-3"
            fill="none"
        ...
        <button
          class="flex-1 h-8 rounded-lg bg-white/[0.05] hover:bg-white/[0.1] text-[11px] font-medium text-white/70 hover:text-white flex items-center justify-center gap-1.5 transition-all"
          @click="sendToBack(layers.find(l => l.id === selectedIds[0])!)"
        >
          <svg
            class="w-3 h-3"
            fill="none"
